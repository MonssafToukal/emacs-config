...
ncluding demos of how they are used.
...
(map! :nivem "C-S-<right>"             evil-insert-state-map))
...
;;(add-to-list 'company-backends 'companmajor-mode)
       'puppet-mode)
...
(defun before-save-hook-go ()
  (when (eq major-mode 'reshed notification."
  ...
  (advice-add 'lsp--on-notification :before-untilterm-mode-hook
...
(set-popup-ru #'ak-lsp-ignore-semgrep-rulesRefreshed))
...
(after! lsp-mode
  (defun ak-lsp-ignore-semgrep-rulesRefreshed (workspace notification)
    "Ignore semgrep/rulesRefy-shell)
...
(defun before-save-hook-puppet ()
  (eql (with-current-buffer (current-buffer) #'evil-window-increase-width)
...
;; This will open documentation for it, i)
